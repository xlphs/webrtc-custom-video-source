...
std::unique_ptr<cricket::VideoCapturer> Conductor::OpenVideoCaptureDevice() {
	...
	// pick FaceTime HD Camera

	cricket::WebRtcVideoDeviceCapturerFactory factory;
	std::unique_ptr<cricket::VideoCapturer> capturer;
	for (const auto& name : device_names) {
		if (name.find("FaceTime") == std::string::npos) {
			continue;
		}
		
		MessageBox(0, L"Using FaceTime camera", L"", MB_OK);
	...
	if (!capturer) {
		MessageBox(0, L"No video capture", L"", MB_OK);
	}
...
void Conductor::AddStreams() {
	...
	rtc::scoped_refptr<webrtc::MediaStreamInterface> stream =
		...
		rtc::scoped_refptr<CustomVideoTrackSource> test(new rtc::RefCountedObject<CustomVideoTrackSource>());
